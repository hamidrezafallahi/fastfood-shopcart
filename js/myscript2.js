...
function createProductBox(name, price, content, group_id, product_id) {
    ...
    a.setAttribute("href", "food_details.html?group=" + (Number(group_id)+1) + "&product=" + (Number(product_id)+1));
    a.appendChild(span1);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // console.log(mydata.product_groups);
    // console.log("url : " + group)
    // console.log(mydata.product_groups[1].group_id);

    for (const group_index in mydata.product_groups) {
        //for (const group_index = 1 ;group_index <= mydata.product_groups;group_index++ ) {
...
function shopcart_counter(){
    ...
    var i =js_get.length;
    var shopcard = document.getElementsByClassName("text-white");
    shopcard[0].innerText=i;
}
shopcart_counter();